...
function load(page) {
  var url = "".concat(apiUrl, "/users?page=").concat(page);
  return fetch(url).then(function (res) {
    if (!res.ok) {
      throw new Error("API returned error code ".concat(res.status));
    }
...
var ProfileCard = /*#__PURE__*/function () {
  ...
  _createClass(ProfileCard, [{
    ...
    value: function render() {
      ...
      section.attr({
        id: "profile_".concat(this.id),
        class: 'profile-card'
      });
      var avatar = new Avatar(this.avatarUrl, this.name);
      var userInfo = new UserInfo(this.name, this.email);
...
function renderUsers() {
  var main = $('main');
  blockButtons();
  load($('#page').text()).then(function (users) {
    ...
    users.forEach(function (user) {
      ...
      var profileCard = new ProfileCard(id, name, email, avatarUrl);
